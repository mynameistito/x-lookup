...
export class MemoryStore implements CacheStore {
  ...
  async set<T>(key: string, value: T, ttlSeconds: number): Promise<void> {
    this.map.set(key, { entry: { value, expiresAt: Date.now() + ttlSeconds * 1000 } })
...
export class CacheApiStore implements CacheStore {
  ...
  async set<T>(key: string, value: T, ttlSeconds: number): Promise<void> {
    try {
      const envelope: CacheEntry<T> = { value, expiresAt: Date.now() + ttlSeconds * 1000 }
      const body = JSON.stringify(envelope)
      await this.cache.put(await this.urlFor(key), new Response(body, {
        headers: {
...
function defaultCacheApi(): MinimalCache | undefined {
  const storage = (globalThis as { caches?: { default?: MinimalCache } }).caches?.default
...
export function memoryConfig(ttlSeconds = DEFAULT_TTL_SECONDS): RuntimeConfig {
  return { ttlSeconds, stores: [memoryStore] }
}
...
export function workerConfig(env: Env): RuntimeConfig {
  ...
  return { ttlSeconds: parseTtlSeconds(env.CACHE_TTL_SECONDS), stores }
}
...
export async function withCache<T>(
  ...
  for (let index = 0; index < config.stores.length; index += 1) {
    ...
    for (let backfill = 0; backfill < index; backfill += 1) {
      const remaining = Math.max(1, Math.ceil((hit.expiresAt - Date.now()) / 1000))
  ...
  await Promise.all(config.stores.map((store) => store.set(key, value, config.ttlSeconds)))
