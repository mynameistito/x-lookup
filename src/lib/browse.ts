...
export async function browse(input: BrowseInput, config: RuntimeConfig = memoryConfig()): Promise<BrowseResult> {
  ...
  const cached = await withCache(key, truthy(input.nocache), () => browseUncached(input, resource, page, limit), config)
