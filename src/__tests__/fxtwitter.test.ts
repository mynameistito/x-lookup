...
describe('getParentStatusId', () => {
  ...
  test('returns replying_to_status[0] when replying_to is null or undefined', () => {
    expect(getParentStatusId(makeTweet('200', { replying_to: null, replying_to_status: ['100'] }))).toBe('100')
    expect(getParentStatusId(makeTweet('200', { replying_to_status: ['100'] }))).toBe('100')
  })

  test('returns undefined when parent identity is absent', () => {
    expect(getParentStatusId(makeTweet('100'))).toBeUndefined()
    expect(getParentStatusId(makeTweet('200', { replying_to: { screen_name: 'alice' } as FxReplyingTo }))).toBeUndefined()
    expect(getParentStatusId(makeTweet('200', { replying_to: [], replying_to_status: null }))).toBeUndefined()
  })
...
describe('fetchFxConversationChain', () => {
  ...
  test('walks the parent chain root-first across long threads', async () => {
    ...
    vi.stubGlobal('fetch', vi.fn().mockImplementation((url: string) => {
      ...
      return Promise.resolve(new Response(JSON.stringify(fxStatusResponse(tweet)), { status: 200 }))
    }))
...
describe('fetchFxFullThread', () => {
  ...
  test('returns single tweet directly when it has no parent', async () => {
    ...
    vi.stubGlobal('fetch', vi.fn().mockImplementation((url: string) => {
      if (url.includes('/thread/')) {
        return Promise.resolve(new Response(JSON.stringify({ code: 200, thread: [tweet] }), { status: 200 }))
      }
      return Promise.resolve(new Response(JSON.stringify(fxStatusResponse(tweet)), { status: 200 }))
    }))
  ...
  test('falls back to conversation chain when thread endpoint returns single tweet with parent', async () => {
    ...
    vi.stubGlobal('fetch', vi.fn().mockImplementation((url: string) => {
      if (url.includes('/thread/')) {
        return Promise.resolve(new Response(JSON.stringify({ code: 200, thread: [leaf] }), { status: 200 }))
      }
      if (url.includes('/status/100')) {
        return Promise.resolve(new Response(JSON.stringify(fxStatusResponse(root)), { status: 200 }))
      }
      return Promise.resolve(new Response(JSON.stringify(fxStatusResponse(leaf)), { status: 200 }))
    }))
...
describe('upstream refusal gating', () => {
  ...
  test('search maps upstream NOT_FOUND to 502 search_unavailable, never a fake 404', async () => {
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue(
      new Response(JSON.stringify({ code: 404, message: 'NOT_FOUND' }), { status: 200 }),
    ))

    const error = await searchFxStatuses('cloudflare', 'latest').catch((e: unknown) => e)
  ...
  test('search still returns real results when upstream cooperates', async () => {
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue(
      new Response(JSON.stringify({ code: 200, results: [makeTweet('1')], cursor: { bottom: 'next' } }), { status: 200 }),
    ))

    const result = await searchFxStatuses('cloudflare', 'latest')
    expect(result.results.map((tweet) => tweet.id)).toEqual(['1'])
  ...
  test('profile lookups keep truthful 404s for genuinely missing users', async () => {
    ...
    const error = await fetchFxProfile('nobody').catch((e: unknown) => e)
    expect(error).toBeInstanceOf(ConvertError)
  ...
  test('non-JSON upstream bodies surface as 502 instead of crashing', async () => {
    ...
    const error = await fetchFxProfile('anyone').catch((e: unknown) => e)
    expect(error).toBeInstanceOf(ConvertError)
    expect((error as ConvertError).status).toBe(502)
