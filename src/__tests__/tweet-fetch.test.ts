...
vi.mock('../lib/syndication.js', () => ({
  fetchSyndicationStatus: vi.fn(),
}))

import { fetchPosts } from '../lib/tweet-fetch.js'
import { fetchFxConversationReplies, fetchFxFullThread, fetchFxStatus } from '../lib/fxtwitter.js'
...
describe('fetchPosts provider fallbacks', () => {
  ...
  test('private posts short-circuit without trying the fallback', async () => {
    vi.mocked(fetchFxStatus).mockRejectedValue(new ConvertError(404, 'Post is private.', 'private_tweet'))

    await expect(fetchPosts('alice', '123', 'off')).rejects.toMatchObject({ code: 'private_tweet' })
    expect(fetchSyndicationStatus).not.toHaveBeenCalled()
  })

  test('surfaces the last provider error when every attempt fails', async () => {
    vi.mocked(fetchFxStatus).mockRejectedValue(new ConvertError(502, 'fx down', 'fxtwitter_error'))
    vi.mocked(fetchSyndicationStatus).mockRejectedValue(new ConvertError(502, 'syndication down', 'syndication_error'))

    const error = await fetchPosts('alice', '123', 'off').catch((e: unknown) => e)
  ...
  test('reports a truthful 404 when a provider confirms the post is missing', async () => {
    vi.mocked(fetchFxStatus).mockRejectedValue(new ConvertError(404, 'Post not found.', 'not_found'))
    vi.mocked(fetchSyndicationStatus).mockRejectedValue(new ConvertError(502, 'syndication down', 'syndication_error'))
