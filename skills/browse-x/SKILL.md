...
allowed-tools:
  - Bash(node skills/browse-x/scripts/browse-x.ts *)
...
```bash
node skills/browse-x/scripts/browse-x.ts \
  "https://x.com/handle/status/1234567890"

node skills/browse-x/scripts/browse-x.ts status \
  "https://x.com/handle/status/1234567890" \
...
```bash
node skills/browse-x/scripts/browse-x.ts profile @handle --limit 20
node skills/browse-x/scripts/browse-x.ts "https://x.com/handle" --full
node skills/browse-x/scripts/browse-x.ts followers handle --limit 50
node skills/browse-x/scripts/browse-x.ts following handle --page 2 --full
```
...
```bash
node skills/browse-x/scripts/browse-x.ts search "from:handle release" --feed latest
node skills/browse-x/scripts/browse-x.ts search "typescript" --feed media --page 3 --limit 10
node skills/browse-x/scripts/browse-x.ts search "typescript" --cursor '<opaque cursor>'
```
