...
- Deploy: `bun run deploy` (`alchemy deploy --stage prod --yes --adopt --profile mynameistito`)
...
- Auth: local commands use `--profile mynameistito` (`~/.alchemy/profiles.json`); CI uses the `CLOUDFLARE_API_TOKEN` / `CLOUDFLARE_ACCOUNT_ID` repository secrets referenced by name only.
...
- CI: `.github/workflows/ci.yml` runs credential-free checks (install frozen lockfile, Ultracite, typecheck, tests, `alchemy:check`). `.github/workflows/deploy.yml` validates again, then deploys `prod` on pushes to `main` and isolated `pr-<number>` preview stacks for same-repo pull requests; preview stacks are destroyed when the PR closes (with a guard refusing to destroy `prod`). Fork pull requests never receive credentials.
