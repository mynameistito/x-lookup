...
Preview deployments and cleanup require GitHub Environment approval, while production deployments proceed automatically after successful CI on `main`. In repository **Settings -> Environments**, create `production` with a `main` deployment branch policy and no required reviewers, and `preview` plus `preview-cleanup` with trusted maintainers or admins as required reviewers; disable self-review and administrator bypass if available. The `workflow_run` deployment workflow checks out the exact CI-passed SHA. Cleanup checks out only the trusted default branch, destroys `pr-<number>` first, and removes GitHub deployment records only after successful teardown; failed cleanup retains those records and the diagnostic run.
