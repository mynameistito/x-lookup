...
Preview deployments and production deploy automatically after successful CI; only destructive preview cleanup uses the protected `preview-cleanup` environment. In repository **Settings -> Environments**, protect `preview-cleanup` with trusted maintainers or admins as required reviewers; disable self-review and administrator bypass if available. The `workflow_run` deployment workflow checks out the exact CI-passed SHA. Cleanup checks out only the trusted default branch, destroys `pr-<number>` first, and removes GitHub deployment records only after successful teardown; failed cleanup retains those records and the diagnostic run.
